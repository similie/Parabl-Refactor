    ...
    (err, passport) => {
      ...
      if (!req.user) {
        ...
        // Action:   Get the user associated with the passport.
          ...
          if (
            query.hasOwnProperty('tokens') &&
            query.tokens !== passport.tokens
          ) {
            passport.tokens = query.tokens;
          }

          // Save any updates to the Passport before moving on
          // [sg] refactor to remove unused callback [passport] in sails.save v0.12
          passport.save(err => {
            if (err) {
              return next(err);
            }
...
passport.loadStrategies = function() {
  ...
  Object.keys(strategies).forEach(key => {
    ...
    if (key === 'local') {
    ...
    } else if (key === 'JwtBearer') {
      const secret = sails.config.session.secret;
      Strategy = strategies[key].strategy;
      self.use(new Strategy(secret, self.protocols.jwt.login));
    } else if (key === 'jwt') {
      const secret = sails.config.session.secret;
      ...
      Strategy = strategies[key].strategy;
      self.use(new Strategy(options, self.protocols.jwt.login));
    } else {
...
passport.disconnect = function(req, res, next) {
  ...
  Passport.findOne(query, (err, passport) => {
    ...
    // [sg] fix unused error variable (was using err, above)
