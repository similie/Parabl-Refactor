...
const { isProd } = require('../../builds/utils');
...
const Passport = {
  ...
  attributes: {
    ...
    protocol: { type: 'alphanumeric', required: true, defaultsTo: 'local' },
    ...
    validatePassword: function(password, next) {
      return new Promise((resolve, reject) => {
        bcrypt.compare(password, this.password, (err, compares) => {
          if (err) {
            reject(err);
            return (next || _.noop)(err);
          }

          resolve(compares);
          (next || _.noop)(err, compares);
        });
      });
    },

    toJSON: function() {
      const passport = this.toObject();
  ...
  afterCreate: function(values, next) {
    ...
    req.method = 'POST';
    User.findOneById(values.user).exec((err, user) => {
      if (err || !user) {
        return;
      }
      req.user = user;
      UserActivity.set(UserActivity.USER_ACCOUNT_ACTIVATED, null, req);
    });
  ...
  terminateV15Session: function(req, res) {
    const sessionValues = this.v15Session();
    const options = this.cookieOptions();
    res.clearCookie(sessionValues.JWT_TOKEN, options);
    res.clearCookie(sessionValues.REFRESH_TOKEN, options);
    return this.removeRefreshToken(req.user);
  },

  cookieOptions: function() {
    return {
      // domain: process.env.COOKIE_DOMAIN || '.similie.org',
      secure: isProd()
    };
  },

  removeRefreshToken: function(user) {
    return this.update(
      { user: user.id, inactive: false },
      { accessToken: null }
    );
  },
  ...
  v15Session: function() {
    return {
      JWT_TOKEN: 'parabl.token',
      REFRESH_TOKEN: 'parabl.refresh'
    };
  }
};
